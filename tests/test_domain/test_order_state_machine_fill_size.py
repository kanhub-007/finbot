...
class TestFillSizeParam:
    def test_partial_fill_increments_via_explicit_param(self):
        lc = OrderLifecycle(order_id="o", symbol="BTC", side="buy", original_size=Decimal("1"))
        ...
        OrderStateMachine.transition(lc, OrderState.PARTIALLY_FILLED, reason="fill 1", fill_size=Decimal("0.3"))
        assert lc.filled_size == Decimal("0.3")
        OrderStateMachine.transition(lc, OrderState.PARTIALLY_FILLED, reason="fill 2", fill_size=Decimal("0.4"))
    ...
    def test_reason_string_no_longer_parsed_for_size(self):
        ...
        OrderStateMachine.transition(lc, OrderState.ACCEPTED)
        OrderStateMachine.transition(lc, OrderState.OPEN)
        OrderStateMachine.transition(lc, OrderState.PARTIALLY_FILLED, reason="user cancelled trigger", fill_size=Decimal("0.5"))
